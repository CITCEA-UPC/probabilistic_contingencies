function [MMC_trafo_SS,ufinal] = build_STAT_trafo(nodeAC,number,Connectivity_Matrix,f,R,L)
    ...
    unus{j}   = join(['STAT',num2str(number),'.idiffq']); 
    unus{j+1} = join(['STAT',num2str(number),'.idiffd']);
    Dnus(:,j:j+1) = [1 0;0 1]; 
    ...
    Cnus = [0;0];
    ynus = { join(['STAT',num2str(number),'.itrafoq']) ;...
             join(['STAT',num2str(number),'.itrafod']) };
    xnus={''};
...
%     Cl = [1 0; 0 1];
%     Dl = [0 0; 0 0];
%     xl = {join(['STAT',num2str(number),'.iltrafoq']);...
%           join(['STAT',num2str(number),'.iltrafod'])};
%     ul = {join(['NET','.vn',num2str(nodeAC),'q'])  ;... 
%           join(['NET','.vn',num2str(nodeAC),'d'])} ;
%     yl = {join(['STAT',num2str(number),'.iltrafoq']);...
%           join(['STAT',num2str(number),'.iltrafod'])};
...
%     unus_rl = { join(['STAT',num2str(number),'.itrafoq'])  ;...
%                 join(['STAT',num2str(number),'.itrafod'])  ;...
%                 join(['STAT',num2str(number),'.iltrafoq']) ;...
%                 join(['STAT',num2str(number),'.iltrafod'])};
%     ynus_rl = {join(['STAT',num2str(number),'.irtrafoq']) ;...
%                join(['STAT',num2str(number),'.irtrafod'])};
%     SS_nus_rl = ss(Anus_rl,Bnus_rl,Cnus_rl,Dnus_rl,'StateName',{''},'InputName',unus_rl,'OutputName',ynus_rl);
    ...
    Cr = [0;0];
    Dr = R*[1 0 ; 0 1];
%     ur = {join(['STAT.',num2str(number),'irtrafoq']) ;...
%                join(['STAT.',num2str(number),'irtrafod'])};
    ur = {join(['STAT',num2str(number),'.itrafoq']) ;...
               join(['STAT',num2str(number),'.itrafod'])};
