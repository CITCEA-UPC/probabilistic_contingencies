...
function [Connectivity_Matrix,connect_mtx_PI,connect_mtx_rl, T_nodes] = generate_general_connectivity_matrix(T_NET,T_trafo, T_load, T_TH, T_SG, T_STATCOM, T_VSC, T_MMC_Pac_GFll,T_MMC_Vdc_GFll, T_b2b, T_user)
    ...
    nsb=zeros(1,tn);
    for node = 1:1:tn
        nsb(node)=sum(sum([T_load.bus==node; T_TH.bus==node; T_SG.bus==node; T_STATCOM.bus==node; T_VSC.bus==node; T_MMC_Vdc_GFll.NodeAC==node; T_MMC_Pac_GFll.NodeAC==node; T_user.bus==node]));
    end
    nsb=max(nsb);
    ...
    mmc_list = {T_MMC_Vdc_GFll, T_MMC_Pac_GFll};

    for idx = 1:1:length(mmc_list)
        T_mmc = mmc_list{idx};
        for mmc =1:1:size(T_mmc.NodeAC,1)
            for i =2:1:nsb+2
                if ismissing(T_nodes(T_mmc.NodeAC(mmc),i))
                    T_nodes{T_mmc.NodeAC(mmc),i} = join(["MMC",num2str(T_mmc.number(mmc))]);
                    break
                end
            end
        end
    ...
    xx_list = {T_load, T_TH, T_SG, T_STATCOM, T_VSC, T_user};
    xx_names = ["Load", "TH", "SG", "STATCOM", "VSC", "user"];
