function [NET] = generate_general_rl_NET(Connectivity_Matrix,T_nodes,PI_T_nodes,T_NET)
...
outputs = [];
 ...
 for i=1:1:size(T_nodes.Node,1)
     ...
     if ((isempty(strings) & not(ismember(i,PI_T_nodes.Node)))) | (contains(strings,"TH") & not(ismember(i,PI_T_nodes.Node))) %Chech if there is something connected to the node
...
for i = 1:1:size(T_NET.NodeA,1)
    ...
    NET.RL(i).SS=ss_rl{i};
    if ismissing(T_nodes(T_nodes.Node==T_NET.NodeB(i),2:end)) & not(PI_T_nodes.Node==T_NET.NodeB(i))
            internal_node=true;
