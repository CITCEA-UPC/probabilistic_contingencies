                        ...
                        for n_j_j = 2:1:size(T_nodes,2)
                            if not(ismissing(T_nodes{n_i_i,n_j_j}))
                                ...
                                if isequal(element,"MMC")
                                    ...
                                    str = split(T_nodes{n_i_i,n_j_j});
                                    number =str(2);
                                    llista_u_nus = [llista_u_nus; {join(['MMC',num2str(number),'.idiffq'])} ; {join(['MMC',num2str(number),'.idiffd'])} ];
                        
                                    %Definim una Din nova pel nus:
                                    Din = [Din [1 0; 0 1]]; %(Positiu perquè entra al nus)
                
                                    %Definim entrada global del sistema
                                    llista_u_AC = [llista_u_AC ; {join(['MMC',num2str(number),'.idiffq'])} ; {join(['MMC',num2str(number),'.idiffd'])}];
                                end
                ...
                for jj = 1 : 1 : size(Connectivity_Matrix_PI,2)
                        ...
                        if Connectivity_Matrix_PI(ii,jj) == 1
                            ...
                            elseif jj>ii
                                ...
                                rl_x = [ {join( ['iq',num2str(ii+minimum_node),num2str(jj+minimum_node)])} ; {join( ['id',num2str(ii+minimum_node),num2str(jj+minimum_node)])}];
