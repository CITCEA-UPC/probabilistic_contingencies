function T_MMC_Pac = generate_SS_MMC_Pac(T_MMC_Pac,lp_mmc,Connectivity_Matrix)

    for mmc = 1:1:height(T_MMC_Pac)
        lp = lp_mmc{mmc};
        T_MMC = T_MMC_Pac(mmc,:);
        %% Outer Loops:
        Pac_control = build_Pac_control(T_MMC.kpPac, T_MMC.kiPac, lp.idiffq0_c,lp.idiffd0_c,lp.vnq0_c,lp.vnd0_c,T_MMC.number,T_MMC.NodeAC);
        ...
        if isequal(T_MMC.trafo{:},'Yes')
            if isequal(T_MMC.join_trafo{:},'Yes')
                ...
                u = {join(['VSC',num2str(T_MMC.number),'.Pac_ref'])                        ;...
            ...
            u = {join(['NET','.vn',num2str(T_MMC.NodeAC),'q'])                         ;... 
                 join(['NET','.vn',num2str(T_MMC.NodeAC),'d'])                         ;...
                 join(['VSC',num2str(T_MMC.number),'.Pac_ref'])                        ;...
                 join(['VSC',num2str(T_MMC.number),'.Q_ref'])                          ;...
                 join(['VSC',num2str(T_MMC.number),'.Et_ref'])                         ;...
                 join(['VSC',num2str(T_MMC.number),'.angle_ref'] )                     ;...       
                 join(['DC_NET.v',num2str(T_MMC.NodeDC),'DC']);                        };
