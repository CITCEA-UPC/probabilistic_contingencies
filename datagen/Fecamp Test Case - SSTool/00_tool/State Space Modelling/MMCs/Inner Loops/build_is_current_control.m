function is_current_control = build_is_current_control( kp, ki, Lc,La ,f, number,nodeAC)
    ...
    x = { join(['VSC',num2str(number),'.PI_idiffq']) ;...
          join(['VSC',num2str(number),'.PI_idiffd'])};

    u = { join(['VSC',num2str(number),'.idiffq_ref'])                               ;... 
          join(['VSC',num2str(number),'.idiffd_ref'])                               ;...
    ...
    y = {join( ['VSC',num2str(number),'.vdiff_q_c',num2str(number)] ) ; ...
         join( ['VSC',num2str(number),'.vdiff_d_c',num2str(number)] )};
