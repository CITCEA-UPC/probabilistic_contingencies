...
function [DC_NET] = generate_DC_NET(DC_Connectivity_Matrix,T_DC_nodes,T_DC_NET,T_MMC_Pac_GFll,T_MMC_Vdc_GFll)
...
%             end
         Cn = Cn + sum( [T_DC_NET.C(T_DC_NET.bus_from == i & T_DC_NET.bus_to == Node ), T_DC_NET.C(T_DC_NET.bus_from == Node & T_DC_NET.bus_to == i )] );
         Gn = Gn + sum( [T_DC_NET.G(T_DC_NET.bus_from == i & T_DC_NET.bus_to == Node ), T_DC_NET.G(T_DC_NET.bus_from == Node & T_DC_NET.bus_to == i )] );
    ...
    %for n_i_i = 1 : 1 : size(T_DC_nodes,1)
         if isequal(T_DC_nodes.Element_1{i},'MMC_Pac_GFll')
                llista_u_nus_DC = [llista_u_nus_DC; { join( ['VSC',num2str(T_MMC_Pac_GFll.number(T_MMC_Pac_GFll.NodeDC == i)),'.iDC'] ) }];
                Din = [Din [1]]; %(Positiu perquè entra al nus) 
                llista_u_DC = [llista_u_DC ; { join( ['VSC',num2str(T_MMC_Pac_GFll.number(T_MMC_Pac_GFll.NodeDC == i)),'.iDC'] ) }];
         end
         if isequal(T_DC_nodes.Element_1{i},'MMC_Vdc_GFll')
                llista_u_nus_DC = [llista_u_nus_DC; { join( ['VSC',num2str(T_MMC_Vdc_GFll.number(T_MMC_Vdc_GFll.NodeDC == i)),'.iDC'] ) }];
                Din = [Din [1]]; %(Positiu perquè entra al nus) 
                llista_u_DC = [llista_u_DC ; { join( ['VSC',num2str(T_MMC_Vdc_GFll.number(T_MMC_Vdc_GFll.NodeDC == i)),'.iDC'] ) }];
        end
    %end
