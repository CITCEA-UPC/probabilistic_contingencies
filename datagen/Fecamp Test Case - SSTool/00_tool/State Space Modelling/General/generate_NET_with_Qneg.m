...
%   - RL/RC or R load: R can be either be a CONSTANT or an INPUT variable
...
%   - RC load: R can only be a CONSTANT. R as INPUT is not implemented yet
...
for l=1:1:size(T_load,1)
    ...
    if bus <= size(connect_mtx_PI,1) && sum(connect_mtx_PI(bus,:)) % Load is connected to any PI-line
          ...
          if T_load.L(l) == 0 &&  T_load.C(l) == 0% R Load 
          ...
          elseif T_load.C(l)~=0 % RC load
              load = build_LoadC_in_PI_addR(T_load.bus(l),T_load.number(l),T_global.f_Hz(1),T_load.R(l),T_load.C(l),results.bus(results.bus.bus==bus,:),delta_slk); % R is INPUT
          end
        ...
        elseif T_load.C(l)~=0 % RC load
            load = build_LoadC_in_rl(T_load.bus(l),T_load.number(l),connect_mtx_rl,T_nodes,T_global.f_Hz(1),T_load.R(l),T_load.C(l)); % R is CONSTANT 
        end
...
if ~(isempty(T_DC_NET))
    ...
    warning('DC NET is not yet implemented')
    l_blocks{end+1} = DC_NET;
end

%% Display system graph

generate_NET_graph(connect_mtx, T_trafo, T_DC_NET, T_NET, T_nodes, T_load, T_TH, T_SG, T_VSC, T_user)
