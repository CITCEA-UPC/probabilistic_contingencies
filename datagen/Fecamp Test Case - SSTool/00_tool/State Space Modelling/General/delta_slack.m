...
for idx = 1:height(T_global)
    element_slk = T_global{idx,"ref_element"}{:};
    bus_slk = T_global{idx,"ref_bus"};
    ...
    switch element_slk
        ...
        case 'SG'
            num_slk     = T_SG.number(T_SG.bus == bus_slk);
            delta_slk   = sg_slack(T_SG(num_slk,:), T_global{idx,"Sb"});
        case 'GFOR'
            num_slk     = T_VSC.number(T_VSC.bus == bus_slk & strcmp(T_VSC.mode,'GFOR'));
            delta_slk   = gfor_slack(T_VSC(num_slk,:), T_global{idx,"Sb"});
        case 'user'
            num_slk     = T_user.number(T_user.bus == bus_slk);
            T_XX        = T_user(num_slk,:);
    ...
    element_slk = T_global{1,"ref_element"}{:};
    bus_slk = T_global{1,"ref_bus"};
    num_slk = T_global{1,'num_slk'};
    delta_slk = T_global{1,'delta_slk'};
end    
