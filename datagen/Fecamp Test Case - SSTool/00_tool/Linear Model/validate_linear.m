...
if isempty(list_pqv) % RL NET or no elements. Assume only voltages
    ...
    for idx = 1:length(list_buses)
        ...
        vq0   = vq_nolin(find(time_sim>=tstep-0.01,1)); 
        vd0   = vd_nolin(find(time_sim>=tstep-0.01,1)); 
        vq_lin   = out_lin.(['NET_vn' num2str(bus) 'q']) + vq0*ones(nSamples,1);
    ...
    for idx = 1:height(list_pqv)
       ...
       figure(f_pq)
        ...
        if elementName ~= "Load" && elementName ~= "TH"
           ...
           figure(f_w)  
               ...
               if num==num_slk && convertCharsToStrings(elementName) == convertCharsToStrings(element_slk)
                   w_lin    = out_lin.REF_w + w0; % slack !!!!!!!!
               else
