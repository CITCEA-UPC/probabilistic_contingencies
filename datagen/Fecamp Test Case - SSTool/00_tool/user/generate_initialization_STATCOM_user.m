function ini_stat = generate_initialization_STATCOM_user(T_STATCOM,results) 
...
ini_stat = cell(1,height(T_STATCOM));
    ...
    for stat = 1:1:height(T_STATCOM)
        ...
        num         = T_STATCOM.number(stat);
        theta0      = results.global.theta(bus)*pi/180;
        U_PCC_VSC   = results.global.Vm(bus)/sqrt(3);  %(Vll rms) PCC Voltage Magnitude sqrt(uq^2+ud^2)/sqrt(2) 
